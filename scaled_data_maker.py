...
def min_max_rescale(tensor):
    #se serve poi la metti in functions
...
temperature = data[0, ...]
del data
temperature_rescaled = min_max_rescale(temperature)
del temperature
temperature_centered = temperature_rescaled - np.mean(temperature_rescaled, axis=1, keepdims=True)
del temperature_rescaled
print('T done')
...
filtered_data = temperature_centered[mask, :]
...
np.save("binned_temperature_cenetered_scaled.npy", tensor_data)
